...
int main() {
	while (1) {
		...
		if (num == 1) { //1. 좌석 예약시스템
			...
			cin >> y;
			sum_y += y; //입력된 y값을 누젛가여 총 청소년 인원수를 구함
			...
			for (int i = 0; i < x + y; ++i) { // 입력한 성인 인원수와 청소년 인원수 만큼 성공할 때까지 반복
				...
				cout << endl;
				if (a < 11 && b < 7) { //행과 열을 범위에 맞게 입력했을때
			...
			for (int o = 0; o < z; ++o) { //변경할 좌석수만큼 성공할때까지 반복
				...
				if (a < 11 && b < 7) {//현재좌석을 범위에 맞게 입력했을 때
					if (seat[a - 1][b - 1] == 1) {//현재 좌석을 알맞게 입력했을 때
						seat[a - 1][b - 1] = 0; //현재 좌석을 빈자리 바꿈
						cout << "변경 좌석(n열, m번째): ";
						...
						if (a < 11 && b < 7) { //변경 좌석을 범위에 맞게 입력했을 때
